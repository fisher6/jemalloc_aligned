...
{
	//jemalloc naive implementation inclides runsing to the closest mulitple of a 32/64	
	int aligned_size = (size+(CUSTOM_ALIGNMENT-1))&~(CUSTOM_ALIGNMENT-1);
	void* ret =  je_aligned_alloc(CUSTOM_ALIGNMENT, aligned_size);
	if((unsigned long)ret % CUSTOM_ALIGNMENT != 0){
		printf("Not aligned to %d byte! \n",CUSTOM_ALIGNMENT);
		exit(1);
	}
	return (ret);
}
...
{	
	/*project modification : check that the aligment is a leat CUSTOM_ALIGMENT and
	 change if not */
	int ret;	
	alignment = alignment < CUSTOM_ALIGNMENT ? CUSTOM_ALIGNMENT : alignment;
...
{
	//our implementation to je_calloc	
	int aligned_size = ((size*num)+(CUSTOM_ALIGNMENT-1))&~(CUSTOM_ALIGNMENT-1);
	void* ret =  je_aligned_alloc(CUSTOM_ALIGNMENT, aligned_size);
	if(((int)ret % CUSTOM_ALIGNMENT) != 0){
		printf("Not aligned to %d byte! \n",CUSTOM_ALIGNMENT);
		exit(1);
	}
...
{
	//project implementation to realloc	
	void *ret;
